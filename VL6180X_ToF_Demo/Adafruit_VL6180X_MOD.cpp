...
#include "Adafruit_VL6180X_MOD.h"
#include "Arduino.h"
...
boolean Adafruit_VL6180X::begin(TwoWire *theWire) {
  ...
  // get part-to-part offset value to apply to readings from "ReadRange"
  _p2p_offset = getPartToPartOffset();

  return true;
}

/* ****************  -------------- ADDED BY ME  ---------------**************************/
    void Adafruit_VL6180X::setReadOutAveragingPeriod(uint8_t periods) 
	{
	/* 
    ...
    */
	write8(READOUT__AVERAGING_SAMPLE_PERIOD, periods);
	}
	
   
   void Adafruit_VL6180X::setRangeThresholds(uint8_t low_mm, uint8_t high_mm) {
	  // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	   write8(SYSRANGE__THRESH_LOW, low_mm);
	   write8(SYSRANGE__THRESH_HIGH, high_mm);
   }
   
		
  
  int8_t Adafruit_VL6180X::getPartToPartOffset(void) {
	  // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	  // part-to-part calibrated offset value in mm (signed value -128 to +127mm)
	  return read8(SYSRANGE__PART_TO_PART_RANGE_OFFSET);
  }
  
  
  void Adafruit_VL6180X::setCrosstalkCompensationRate(uint16_t rate) {
	  // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	  write16(SYSRANGE__CROSSTALK_COMPENSATION_RATE, rate);
  }
  
   void Adafruit_VL6180X::setMaxConvergenceTimeMs(uint8_t ms) {
	   // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	   // Value in ms stored in register. Min =2ms for short range, 63ms max system value
	   // Default value of 30 used if out of range
	   if (ms < 2 || ms > 63)  {
		   ms = 30;
	   }
	  write8(SYSRANGE__MAX_CONVERGENCE_TIME, ms);
  }
  
  float Adafruit_VL6180X::readReturnRate(void) {
	  // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	   // read raw 9.7 format Mcps value
	  uint16_t return_rate = read16(RESULT__RANGE_RETURN_RATE);
	  // scale by division of 128 and return Mcps value
	  return  (float)(return_rate) / 128.0;
  }
  
  
  void Adafruit_VL6180X::setRangeIgnore(uint8_t valid_height, uint16_t ignore_threshold) {
	// wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
    // Valid height in mm. Recommended to be 255mm if using range ignore
    write8(VL6180X_SYSRANGE__RANGE_IGNORE_VALID_HEIGHT, valid_height);
  
    // Ignore Threshold  for Signal Return Rate (Mcps) in Mcps 9.7 format
    write16(VL6180X_SYSRANGE__RANGE_IGNORE_THRESHOLD, ignore_threshold);

  }
  
   void Adafruit_VL6180X::RangeIgnoreMode(bool on) {
	   // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	   // need to set/reset bit 1  without affecting other bits
	   uint8_t regVal = read8(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES);
	   if (on) 		  
		   regVal = regVal | 0x02;		// bit 1
	   else // off
		   regVal = regVal & 0xFD;    // ~bit 1
	   write8(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES, regVal);
   }
   
   void Adafruit_VL6180X::SignalToNoiseMode(bool on) {
	   // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	   // need to set/reset bit 4 without affecting other bits
	   uint8_t regVal = read8(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES);
	   if (on) 		  
		   regVal = regVal | 0x10;	// bit 4
	   else // off
		   regVal = regVal & 0xEF; // ~ bit 4
	   write8(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES, regVal);
   }
   
   void Adafruit_VL6180X::EarlyConvergenceMode(bool on) {
	   // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	   // need to set/reset bit 0 without affecting other bits
	   uint8_t regVal = read8(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES);
	   if (on) 		  
		   regVal = regVal | 0x01;	// bit 0
	   else // off
		   regVal = regVal & 0xFE;	// ~bit 0
	   write8(VL6180X_SYSRANGE__RANGE_CHECK_ENABLES, regVal);
   }
   
   uint8_t Adafruit_VL6180X::readRawRange(void) {
	   // wait for device to be ready 
      while (!(read8(VL6180X_REG_RESULT_RANGE_STATUS) & 0x01)) ;
	  
	      // assumes result is ready from ReadRange call prior to calling this function!!!
  
    // read raw range in mm (no cross-talk calibration applied)  
  uint8_t range = read8(RESULT__RANGE_RAW);
  return range;
}
...
void Adafruit_VL6180X::loadSettings(void) {
  ...
  write8(0x0207, 0x01);			// REGISTER NOT IN DATASHEET
  write8(0x0208, 0x01);			// 
  write8(0x0096, 0x00);
  write8(0x0097, 0xfd);
  write8(0x00e3, 0x00);
...
/*!
   ...
   @param apply_p2p_offset if true, will add the factory cal range offset in SYSRANGE__PART_TO_PART_RANGE_OFFSET
    @return Distance in millimeters if valid
*/
/**************************************************************************/

uint8_t Adafruit_VL6180X::readRange(bool apply_p2p_offset) {
  ...
  // Poll  at slow rate until bit 2 is set
    while (!(read8(VL6180X_REG_RESULT_INTERRUPT_STATUS_GPIO) & 0x04)) {
	  delay(30); 	// see Fig 7 of AN4545; 30ms seems to be longest possible range measurement duration
  }
  ...
  if (apply_p2p_offset) {
	   // add part-to-part factory calibrated offset to make reading more accurate
	   range += _p2p_offset;
  }
