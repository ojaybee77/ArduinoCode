...
#ifndef _ADAFRUIT_VL6180X_MOD_H
#define _ADAFRUIT_VL6180X_MOD_H
...
// Extra registers added by Oli Bailey May'23
...
public:
  ...
  // Added by Oli Bailey May'23 as part of dust-on -lens ensrobustness investigation
  void setRangeIgnore(uint8_t valid_height, uint16_t ignore_threshold);
  void RangeIgnoreMode(bool on);
  void SignalToNoiseMode(bool on);   
  void EarlyConvergenceMode(bool on);   
  void setCrosstalkCompensationRate(uint16_t rate);
  float readReturnRate(void);
  uint8_t readRawRange(void);
  void setMaxConvergenceTimeMs(uint8_t ms);
  int8_t  getPartToPartOffset(void);
  void setPartToPartOffset(int8_t p2p_offset);
  void setRangeThresholds(uint8_t low_mm, uint8_t high_mm);
  void setReadOutAveragingPeriod(uint8_t periods);
